...
public function favoriteCar(Request $request, Car $car)
{
    ...
    try {
        if ($user->favorites->contains($car)) {
            ...
            $message = 'Voiture retirée des favoris avec succès.';
        } else {
            $user->favorites()->attach($car);
            $message = 'Voiture ajoutée aux favoris avec succès.';
        }
        ...
        return redirect()->back()->with('error', 'Une erreur s\'est produite lors de l\'ajout aux favoris.');
    }
}
...
public function comment(Request $request, Car $car)
{
    ...
    {
        $totalCars = Car::count();
        $totalRentals = Location::count();
        $totalUsers = User::count();
        // Charts
        $cars = Car::selectRaw('count(*) as count, marque')->groupBy('marque')->get();
        $cars = Car::all();
        $rentals = Location::selectRaw('count(*) as count, status')->where('status', '!=', 'retour')->
        groupBy('status')->get();
        $users = User::selectRaw('count(*) as count, role')->groupBy('role')->get();
        return view('dashboard')
            ->with('totalCars', $totalCars)
            ->with('totalRentals', $totalRentals)
