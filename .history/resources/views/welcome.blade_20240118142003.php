    ...
    @foreach($cars as $car)
        <div class="col mb-4">
            <div class="card h-100">
                <img src="{{ asset($car->photo_path) }}" class="card-img-top h-100" alt="{{ $car->marque }}">
                <div class="card-body">
...
@push('scripts')
    <script>
        function addToFavorites(carId) {
            // Ajoutez ici la logique pour ajouter la voiture aux favoris
            // Vous pouvez utiliser des requêtes AJAX ou rediriger l'utilisateur vers une autre page pour ajouter la voiture aux favoris
            console.log('Ajouter aux favoris :', carId);
        }
    </script>
@endpush
